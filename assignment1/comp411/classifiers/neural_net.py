...
class FourLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        layer2 = np.dot(relu1, W2) + b2
        relu2 = np.maximum(0, layer2)
        layer3 = np.dot(relu2, W3) + b3
        relu3 = np.maximum(0, layer3)
        layer4 = np.dot(relu3, W4) + b4
        scores = layer4
        ...
        loss = scores[np.arange(X.shape[0]), y] / np.sum(scores, axis=1)
        loss = -np.sum(np.log(loss)) / X.shape[0] + reg * (np.sum(np.square(W1)) + np.sum(np.square(W2)) + np.sum(np.square(W3)) + np.sum(np.square(W4)))
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        ...
        softmax[range(X.shape[0]), y] = -(np.sum(scores, axis=1) - scores[range(X.shape[0]), y]) / np.sum(scores, axis=1)
        softmax /= X.shape[0]

        grads['W4'] = np.dot(relu3.T, softmax) + 2 * reg * W4
        grads['b4'] = np.sum(softmax, axis=0)
        
        hidden3 = np.dot(softmax, W4.T)
        hidden3[relu3 == 0] = 0

        grads['W3'] = np.dot(relu2.T, hidden3) + 2 * reg * W3
        grads['b3'] = np.sum(hidden3, axis=0)
        
        hidden2 = np.dot(hidden3, W3.T)
        hidden2[relu2 == 0] = 0

        grads['W2'] = np.dot(relu1.T, hidden2) + 2 * reg * W2
        grads['b2'] = np.sum(hidden2, axis=0)
        
        hidden1 = np.dot(hidden2, W2.T)
        hidden1[relu1 == 0] = 0

        grads['W1'] = np.dot(X.T, hidden1) + 2 * reg * W1
        grads['b1'] = np.sum(hidden1, axis = 0)
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            cur_idx = np.random.choice(num_train, batch_size)
            ...
            for key in self.params:
                curupdate = grads[key] * learning_rate # update amt
                self.params[key] -= curupdate
