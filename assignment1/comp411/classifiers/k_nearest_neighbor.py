...
class KNearestNeighbor(object):
    ...
    def compute_L2_distances_no_loops(self, X):
        ...
        dists = np.sum(np.square(X), axis=1)[:,np.newaxis] - (2 * X @ self.X_train.T) + np.sum(np.square(self.X_train), axis=1)
        dists = np.sqrt(dists)
    ...
    def compute_Cosine_distances_two_loops(self, X):
        ...
        for i in range(num_test):
            for j in range(num_train):
                ...
                dists[i, j] = np.sum(np.dot(self.X_train[j], X[i])) / (np.sqrt(np.sum(np.square(self.X_train[j]))) * np.sqrt(np.sum(np.square(X[i]))))
    ...
    def compute_Cosine_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            dists[i, :] = np.sum(self.X_train * X[i], axis=1) / (np.sqrt(np.sum(np.square(self.X_train), axis=1)) * np.sqrt(np.sum(np.square(X[i]))))
    ...
    def compute_Cosine_distances_no_loops(self, X):
        ...
        dists = np.dot(X, self.X_train.T) / np.sqrt(np.dot(np.sum(np.square(X), axis=1)[:, np.newaxis], np.sum(np.square(self.X_train), axis=1)[:, np.newaxis].T))
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        return dists
