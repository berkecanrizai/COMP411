...
def huber_loss_naive(W, X, y, reg):
    ...
    def huber(margin, arr):
        if margin < 1:
            lossupd = margin**2
            return (arr * 2 * margin), lossupd
        else:
            lossupd = 4 * margin
            return (arr * 4), lossupd
...
def svm_loss_vectorized(W, X, y, reg):
    ...
    score_mx = np.dot(X, W)
    correct_class_scores = score_mx[np.arange(X.shape[0]), y][:, np.newaxis]
    correct_class_scores = score_mx[np.arange(X.shape[0]), y].reshape(X.shape[0], 1)
    margin = np.maximum(0, score_mx - correct_class_scores + 1)
    score_mx[score_mx - correct_class_scores + 1 <= 0] = 0
...
def huber_loss_vectorized(W, X, y, reg):
    ...
    score_mx = np.dot(X, W)
    correct_class_scores = score_mx[np.arange(X.shape[0]), y][:, np.newaxis]
    error = (score_mx - correct_class_scores)
    
    margins = np.zeros(score_mx.shape)
    margins[((error >= 0) & (error <= 1))] = np.square(error)[(error >= 0) & (error <= 1)]
    margins[error > 1] = 4 * error[error > 1]
    ...
    dw = np.zeros(error.shape)
    dw[((error >= 0) & (error <= 1))] = error[((error >= 0) & (error <= 1))] * 2
    dw[error > 1] = 4
    dw[range(X.shape[0]), y] -= np.sum(dw, axis=1)
    dW = np.dot(X.T, dw) / X.shape[0]
    dW += 2 * reg * W
